...
export const formatName = (item = {}, options = {}) => {
  ...
  const surname = item.surname || item.lastName || item.teacher_surname || item.homeroom_teacher_surname || item.homeroom_surname || "";
  const middleName = item.middleName || item.middle_name || "";
  const givenName = item.givenName || item.given_name || item.firstName || item.teacher_given_name || item.homeroom_teacher_given_name || item.homeroom_given_name || "";
